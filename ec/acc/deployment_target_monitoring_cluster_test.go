...
func TestAccDeploymentExternalMonitoring_basic(t *testing.T) {
	resName := "ec_deployment_external_monitoring.basic"
	randomName := prefix + acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum)
	startCfg := "testdata/deployment_basic_external_monitoring.tf"
	updateCfg := "testdata/deployment_traffic_filter_basic_update.tf"
	cfg := testAccDeploymentExternalMonitoringResourceBasic(t, startCfg, randomName, region)
	updateConfigCfg := testAccDeploymentExternalMonitoringResourceBasic(t, updateCfg, randomName, region)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: cfg,
				Check: checkBasicDeploymentExternalMonitoringResource(resName, randomName,
...
func testAccDeploymentExternalMonitoringResourceBasic(t *testing.T, fileName, name, region string) string {
...
func checkBasicDeploymentExternalMonitoringResource(resName, randomDeploymentName string, checks ...resource.TestCheckFunc) resource.TestCheckFunc {
